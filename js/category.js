...
function renderLanguageFilters(books, selectedCategory, selectedLanguages = [], onFilterChange) {
  const filteredBooks = books.filter(book => book.subSubCategory === selectedCategory);
...
function renderBooks(books, selectedCategory) {
  ...
  renderLanguageFilters(books, selectedCategory, selectedLanguages, () => renderBooks(books, selectedCategory));
  renderPriceFilter(books, selectedCategory);

  let filtered = books.filter(book => book.subSubCategory === selectedCategory);
  ...
  filtered = filtered.filter(book => book.price >= minVal && book.price <= maxVal);

  if (filtered.length === 0) {
    categoryWay.innerHTML = "";
    mainTitle.innerText = "";
    bookList.innerHTML = "<p class='text-gray-500'>Bu filtrə uyğun kitab yoxdur.</p>";
    return;
  }
    ...
    const card = `
      <div class="bg-white border border-transparent rounded-2xl w-full px-3 py-3 hover:shadow-[0px_4px_20px_0px_rgba(0,0,0,0.2)] transition-all duration-300 relative group">
        <a href="details.html?id=${book.id}" onclick="incrementView(${book.id})">
          <img class="w-full h-[340px] object-cover rounded-t-lg" src="${book.cover}" alt="${book.title}" />
        </a>
